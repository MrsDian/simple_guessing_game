...
word_list = ['год', 'человек', 'время', 'дело', 'жизнь', 'день', 'рука', 'раз', 'работа', 'слово', 'место', 'лицо',
            ...
            'море', 'требование', 'основание', 'половина', 'роман', 'круг', 'анализ', 'стихи', 'автомобиль', 'экономика', 'литература', 'бумага', 'поэт', 'степень', 'господин', 'надежда', 'предмет', 'вариант',
            ...
            'карман', 'слава', 'кухня', 'определение', 'функция', 'войско', 'комиссия', 'применение', 'капитан','работник', 'обеспечение', 'офицер', 'фамилия', 'предел', 'выборы', 'ученый', 'бутылка', 'бой', 'теория',
            ...
            'заседание', 'толпа', 'больница', 'существо', 'свойство', 'долг', 'поколение', 'животное', 'схема','усилие', 'отличие', 'остров', 'противник', 'волна', 'реализация', 'страница', 'формирование', 'житель',
            ...
            'оборона', 'выступление', 'температура', 'перспектива', 'подруга', 'приказ', 'жертва', 'ресторан','километр', 'спор', 'вкус', 'признак', 'промышленность', 'американец', 'лоб', 'заключение', 'восток',
            ...
            'устройство', 'вещество', 'категория', 'сезон', 'гостиница', 'издание', 'объединение', 'темнота','человечество', 'колесо', 'опасность', 'разрешение', 'воздействие', 'коллектив', 'камера', 'запас',
...
def display_hangman(tries):
    ...
    guess = input()
    while not guess.isalpha() or len(guess) < len(word) and len(guess) > 1:
        print("Введите букву или слово целиком")
    if len(guess) == 1:
        for i in word:
            if guess == i:
...
def jerk_invade(st, word):
    while not(len(st) == 1 or len(st) == len(word)): 
        print("Брат, давай без рофлов")
        st = input()
...
def play():
    ...
    print("Слово:", word_guessed)
    print("Состояние Mori:")
    print(display_hangman(tries))
    # print(word)
    while tries > 0:
        print("Введите букву или слово целиком")
        st = jerk_invade(input(), word)
        if st in word_guessed:
            print("Эту букву Вы уже отгадали! Попробуйте еще!")
            continue
        word_guess = ""
        if len(st) == 1 and st in word:
            for i in range(len(word)):
                if word[i] == st:
                    word_guess += word[i]
                elif word[i] == word_guessed[i]:
                    word_guess += word[i]
                else:
                    word_guess += "_"
            word_guessed = word_guess
            print("Верно!")
            print(word_guess)
        if word_guess.isalpha() and word_guess == word:
            print('Поздравляем, вы угадали слово! Вы победили!')
            return True
        elif st == word:
            print("Поздравляем, вы угадали слово! Вы победили!") 
            ...
            if tries > 4:
                ...
                print("Осталось", tries, "попытки")  
                print(display_hangman(tries))
            elif tries == 1:
                print("Осталась", tries, "попытка")
                print(display_hangman(tries))
    print(display_hangman(tries))
    print("Загаданное слово:", word)
    print("Да, Mori погиб, но в следующий раз всё получится!")
